...
class Vector {
    int* p = nullptr; // Указатель на массив зн век
    int n = 0; // Размер век

public:
    Vector(int* p, int n) { // Конструктор для создания век из массива зн
        this->n = n;
        this->p = new int[n];
        for (int i = 0; i < n; i++) this->p[i] = p[i];
    }

    Vector(int n) : n(n) { // Конструктор для создания пустого век заданного размера
        p = new int[n]();
    }
    ...
    void print() const { // Метод для вывода значений век
        for (int i = 0; i < n; i++)
            cout << p[i] << " ";
        cout << endl;
    }
